      ...
      shimmer.wrap(layer, 'handle', function (orig) {
        ...
        return function (err, req, res, next) {
            if ((isError(err) || typeof err === 'string') && !err[reportedSymbol]) {
              if (typeof err === 'string' && isStrict) {
                console.warn("[strict mode] APM error handler: not setting reportedSymbol on string - therefore could be reported more than once", err);
              } else {
